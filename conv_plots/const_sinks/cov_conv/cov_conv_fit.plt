...
fit f(x) "cov_conv.dat" u (log($1)):(log(abs($2))) via a,b
plot [100:] "cov_conv.dat" u 1:(abs($2)) w lp notitle,\
exp(b)*x**a w l title sprintf("${%.3f} \\times n^{%.3f}$", b, a)

set output "cov_conv_fit_1_3.tex"
set ylabel "$| \\overline{\\sigma_{13}} - \\E{\\sigma_{13}} |$"
fit f(x) "cov_conv.dat" u (log($1)):(log(abs($3))) via a,b
plot [100:] "cov_conv.dat" u 1:(abs($3)) w lp notitle,\
exp(b)*x**a w l title sprintf("${%.3f} \\times n^{%.3f}$", b, a)
